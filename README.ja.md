...
<p align="right">
  ...
  <a href="./README.ja.md"><img src="https://img.shields.io/badge/日本語-現在-blue" alt="日本語"></a>
</p>
...
- 翻訳方向。例：英語から中国語、中国語から英語、フランス語から中国語。
...
epub_pipeline_template_zh_en を使い、00_orchestrator_zh_en.md から現在の完全な工程を実行してください：
...
出典と権利リスクを確認したあと、epub_pipeline_template_zh_en を使って books/ 以下に新しい書籍工程を作成してください。
```
...
書籍制作のための再利用テンプレートです。主に AI が読んで実行するためのフォルダで、人間の参加者が全ファイルを読む必要はほとんどありません。新しい本を作るときは、AI にこのテンプレートを `books/` 以下の新しいフォルダへコピーさせます。テンプレート本体に実際の本のデータを書き込まないでください。

主な内容：

- `README_ZH_EN.md`：テンプレート説明。
- `MASTER_PROMPT_ZH_EN.md`：新しい本を始めるための主プロンプト。
- `PIPELINE_SPEC_ZH_EN.md`：パイプラインとディレクトリ規約。
...
このプロジェクトで作られた翻訳、注記、表紙、組版、EPUB パッケージにも、それぞれ権利状態があります。公開前に、プロジェクト管理者が配布ライセンスを明確にする必要があります。
