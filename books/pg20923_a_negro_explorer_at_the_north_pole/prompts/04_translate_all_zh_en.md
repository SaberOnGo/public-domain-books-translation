...
普通名词、器物名、衣物名、材料名和动作名必须译成中文；不得写成“原文词（中文释义）”，也不得写成“中文词（原文词）”。人名可因音译查证需要在正文首次出现处保留英文原名；普通名词不适用这条人名规则。旧纸书分隔符如 `* * * * *`、`*****`、`----`、`---` 直接删除。
...
Translate common nouns, object names, clothing names, material names, and action terms into Chinese. Do not add original source terms in parentheses for ordinary nouns. Delete printed-book separators such as `* * * * *`, `*****`, `----`, or `---`.
