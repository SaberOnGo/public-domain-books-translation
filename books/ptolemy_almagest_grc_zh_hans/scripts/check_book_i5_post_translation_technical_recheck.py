...
TRANSLATION = PROJECT_ROOT / "chapters" / "translated" / "005_book_i_05_earth_is_central.md"
OUT_JSON = PROJECT_ROOT / "qa" / "pretranslation" / "005_book_i_05_post_translation_technical_recheck.json"
OUT_MD = PROJECT_ROOT / "qa" / "pretranslation" / "005_book_i_05_post_translation_technical_recheck.md"


REQUIRED = [
    {
        "id": "post_translation_recheck_exists",
        "path": "qa/technical/005_book_i_05_earth_is_central.post_translation_technical_recheck.md",
        "contains": [
    ...
    },
    {
        ...
        "contains": [
            ...
            "本章译文依据 `chapters/src/005_book_i_05_earth_is_central.md`",
    ...
    {
        "id": "term_lock_contains_book_i5_terms",
        "path": "qa/technical/mathematical_term_lock.md",
        "contains": [
            "Book I.5 地球居中与观测反证术语",
            "μέση τοῦ οὐρανοῦ",
            "κέντρον σφαίρας",
            "ἄξων",
            "ἰσημερία",
...
def collect_checks() -> tuple[list[dict[str, object]], list[str]]:
    ...
    if marker_hits:
        failures.append(f"Book I.5 body contains unexpected geometry/table/numeric markers: {marker_hits}")
    checks.append({"id": "geometry_table_numeric_markers_absent_for_i5", "status": "PASS" if not marker_hits else "FAIL", "hits": marker_hits})
...
def main() -> int:
    ...
    if failures:
        ...
        return 1
    print("Book I.5 post-translation technical recheck passed; chapter quality gate remains pending")
    return 0
