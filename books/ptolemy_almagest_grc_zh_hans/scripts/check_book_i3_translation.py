...
TRANSLATION = PROJECT_ROOT / "chapters" / "translated" / "003_book_i_03_heaven_moves_spherically.md"
OUT_JSON = PROJECT_ROOT / "qa" / "pretranslation" / "003_book_i_03_translation_check.json"
OUT_MD = PROJECT_ROOT / "qa" / "pretranslation" / "003_book_i_03_translation_check.md"


REQUIRED = [
    {
        "id": "translation_file_exists",
        "path": "chapters/translated/003_book_i_03_heaven_moves_spherically.md",
        "contains": [
    ...
    },
    {
        ...
        "path": "qa/chapter_controls/003_book_i_03_heaven_moves_spherically.control.md",
        "contains": ["PASS_FOR_CONTROLLED_TRANSLATION_PREP__NOT_TRANSLATED"],
    },
    {
        "id": "technical_audit_exists",
        "path": "qa/technical/003_book_i_03_heaven_moves_spherically.technical_audit.md",
...
FORBIDDEN_FILES = [
    "chapters/final/003_book_i_03_heaven_moves_spherically.md",
    "output/book.epub",
]
...
def collect_checks() -> tuple[list[dict[str, object]], list[str]]:
    ...
    note_count = sum(translation_text.count(f"{index}.") for index in range(1, 7))
    if note_count < 6:
        failures.append("chapter notes appear incomplete")
    checks.append({"id": "chapter_notes_present", "status": "PASS" if note_count >= 6 else "FAIL", "note_count": note_count})

    body_paragraph_count = body_text.count("\n\n")
    if body_paragraph_count < 7:
        failures.append("translation body appears over-compressed")
    checks.append({"id": "proof_chain_not_overcompressed", "status": "PASS" if body_paragraph_count >= 7 else "FAIL", "paragraph_breaks": body_paragraph_count})
...
def write_reports(checks: list[dict[str, object]], failures: list[str], created: str) -> None:
    ...
    for check in checks:
        detail = check.get("path", check.get("note_count", check.get("paragraph_breaks", "")))
...
def main() -> int:
    ...
    if failures:
        ...
        return 1
    print("Book I.3 translation check passed; review remains pending")
    return 0
