...
OUT_JSON = PROJECT_ROOT / "qa" / "pretranslation" / "003_book_i_03_post_translation_technical_recheck.json"
OUT_MD = PROJECT_ROOT / "qa" / "pretranslation" / "003_book_i_03_post_translation_technical_recheck.md"


REQUIRED = [
    {
        "id": "post_translation_recheck_exists",
        "path": "qa/technical/003_book_i_03_heaven_moves_spherically.post_translation_technical_recheck.md",
        "contains": [
    ...
    },
    {
        ...
        "contains": [
            ...
            "本章译文依据 `chapters/src/003_book_i_03_heaven_moves_spherically.md`",
    ...
    {
        "id": "review_exists",
        "path": "reviews/chapters/003_book_i_03_heaven_moves_spherically.review.md",
        "contains": [
    ...
    {
        "id": "term_lock_contains_book_i3_terms",
        "path": "qa/technical/mathematical_term_lock.md",
        "contains": [
            "Book I.3 天球运动与观测反证术语",
            "σφαιροειδῶς φέρεται",
            "αἰεὶ φανεροὶ ἀστέρες",
            "πόλος",
            "ἀναθυμίασις",
            "ὡροσκοπ",
            "αἰθήρ",
...
def collect_checks() -> tuple[list[dict[str, object]], list[str]]:
    ...
    if topic_hits:
        failures.append(f"Book I.3 body contains unexpected geometry/table/numeric markers: {topic_hits}")
    checks.append({"id": "geometry_table_numeric_markers_absent_for_i3", "status": "PASS" if not topic_hits else "FAIL", "hits": topic_hits})
...
def main() -> int:
    ...
    if failures:
        ...
        return 1
    print("Book I.3 post-translation technical recheck passed; chapter quality gate remains pending")
    return 0
