...
OUT_JSON = PROJECT_ROOT / "qa" / "pretranslation" / "010_book_i_10_review_check.json"
OUT_MD = PROJECT_ROOT / "qa" / "pretranslation" / "010_book_i_10_review_check.md"
REQUIRED = [
    ("review_exists", "reviews/chapters/010_book_i_10_chords.review.md", [
    ...
    ("translation_check_exists", "qa/pretranslation/010_book_i_10_translation_check.md", ["PASS_TRANSLATION_DRAFT__REVIEW_PENDING"]),
    ("translation_exists", "chapters/translated/010_book_i_10_chords.md", ["CONTROLLED_DRAFT_TRANSLATION__REVIEW_PENDING"]),
]
...
def write_reports(checks: list[dict[str, object]], failures: list[str], created: str) -> None:
    ...
    lines = ["# Book I.10 评审检查 / Review Check", "", f"check_status: `{status}`", f"created_at_utc: `{created}`", "", "| id | status | path | missing | sha256 |", "|---|---|---|---|---|"]
    for check in checks:
...
def main() -> int:
    ...
    if failures:
        ...
        return 1
    print("Book I.10 review check passed; technical recheck remains pending")
    return 0
