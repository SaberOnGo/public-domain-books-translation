...
status: PASS
created: 2026-05-21T14:33:21Z

### 发布原因 / Release Reason

- 将《天文学大成》Book I.1-I.16 作为独立第一卷 EPUB 候选出版物输出。
- 修复手机分页阅读器中的表格空白页风险，并把前置页 LifeBook URL 改为短文字可点击链接。
...
- `book-info.xhtml` 只保留一条短 LifeBook 入口：`更多：访问 LifeBook`，链接使用蓝色下划线样式，并把 URL 内嵌在文字链接中。
- `book-info.xhtml` 移除 GitHub 项目链接；项目链接、参与方式、QA/prompt 或工作流信息不得进入读者书籍信息页。
...
- 直接显示长 URL 会干扰窄屏排版。
- 前置页缺少模板要求的 LifeBook 访问入口，且旧版同时放入 GitHub 项目链接，违反书籍信息页简洁规则。
...
- `npm run book-i:publish-epubcheck`: PASS，fatal=0, error=0, warning=0
...
- 后续只在用户明确要求时进入 Book II；本版本不触发全 13 卷生产。


## Release v0.0.3 / 版本 v0.0.3

status: PASS
created: 2026-05-21T14:23:36Z
