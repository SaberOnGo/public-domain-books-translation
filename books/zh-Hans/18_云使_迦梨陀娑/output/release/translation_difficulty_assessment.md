...
- 判断说明：本书主要类型为语言学习，检测到 4 章、约 26167 words；专名密度为超高，目标语文体难度为 3/5。

## 复杂度画像

- 主要书籍类型（primary_book_type）：语言学习
- 检测到的类型（detected_book_types）：语言学习、历史、小说、编程、科学
...
- 哲学/理论密度（philosophical_or_theoretical_density）：3/5
...
- 匹配到的相似书籍数量（matched_count）：1
- 历史每 1 万原文单位有效工时（active_hours_per_10k_source_units）：0.32
- 历史每原文单位输入 token（input_tokens_per_source_unit）：1.44
- 历史每原文单位输出 token（output_tokens_per_source_unit）：0.52

- 林伯洛斯特的女孩：相似度 0.065，实际工时 11 小时，总 token 680000，模型等级 high

## 模型建议

- deepseek：建议等级 中（medium）；用途：低成本初译、术语扩展和章节草稿；预估输入 token 68900；预估输出 token 54100。
- gpt：建议等级 高（high）；用途：章节质控、疑难段落、终稿润色和 release 前 QA；预估输入 token 83600；预估输出 token 59000。
- claude：建议等级 高（high）；用途：长上下文一致性复核、风格比较和独立 QA；预估输入 token 73800；预估输出 token 22100。
