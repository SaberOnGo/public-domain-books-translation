...
文件位置说明：本文件是《黑人北极探险家》这一具体书籍工程的精修目标，因此放在本书目录 `books/zh-Hans/1_pg20923_a_negro_explorer_at_the_north_pole/goal/` 下。项目级、语言中立、语言方向专用的可复用规则，分别回填到 `template/epub_pipeline/common/`、`template/epub_pipeline/targets/zh-Hans/` 和 `template/epub_pipeline/en-zh-Hans/`。
...
| `006_*` | 第三章 发现 Rudolph Franke——Whitney 登陆——交易和装煤——与 | 明显截断，且标题中不应放英文首次出现；必须重拟为中文主标题 |
...
- 陌生人名、陌生地名不强制汉译，如 `Professor Marvin`、`Grant Land` 等可在正文保留英文；但章节标题和副标题优先使用中文译名，英文原名放在正文首次出现处或术语表中。
...
- `002` 至 `006` 已完成本轮逐章精修记录。
- `003` 已清理重复标题和误入正文的下一节书名。
- `004` 至 `006` 已完成一轮窄范围正文精修，重点是长段节奏、职业称谓、术语一致性和早期译稿痕迹。
- `008`、`018`、`021`、`023` 已完成本轮结构性精修，重点是超长段拆分、现场叙述节奏、职业称谓统一和明显机器腔修正。
- `011` 已复核标题、称谓和段落节奏，本轮未发现必须改动的正文问题。
- `025` 已修正附录一中不必要的现代口语化表达，保留原书历史视角，不用现代译者声音改写事实判断。
- `026` 附录二决定暂保留预格式化姓名清单，移除误入正文的 Project Gutenberg 转录者说明，并把“名单/索引类附录可豁免普通段落长度门禁，但必须记录理由并验证移动端可读性”的规则回填到模板。
- 已把 Project Gutenberg 转录者说明、OCR 修正说明等数字来源工件不得混入作者正文的规则回填到 common 与 `en-zh-Hans`。
- 后续继续处理全书更细粒度的逐段对照润色，优先关注第 14 至 20 章的冰上行军叙述和 Marvin 段落。
