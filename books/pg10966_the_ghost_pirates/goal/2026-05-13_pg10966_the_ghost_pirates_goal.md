...
本文件定义 `books/pg10966_the_ghost_pirates/` 的 `/goal`。本书工程必须严格遵守仓库根目录 `AGENTS.md`、`template/epub_pipeline/common`、`template/epub_pipeline/en-zh-Hans` 和 `template/epub_pipeline/targets/zh-Hans/` 的规则。所有书籍产物只能写入本目录，禁止写回 `template/`。
