# LifeBook 書坊：全球公版書翻譯與 EPUB 製作協作專案
...
LifeBook 書坊是一個面向全球公版書的多語言翻譯與電子書製作專案。我們希望把已經進入公版、但在某些語言社群還不容易讀到的作品，重新整理、翻譯、審校，並製作成可閱讀的 EPUB。
...
- 想做哪本書。
- 公版來源 URL，如果有的話。
- 翻譯方向，例如英語到西班牙語、法語到日語、中文到英語、日語到德語。
- 使用哪個已有語言模板；如果還沒有對應模板，則先新增對應模板目錄。

例如，可以這樣發給 AI。下面使用「法語到英語」作為具體方向；如果倉庫裡還沒有 `fr-en` 模板，應先新增這個語言方向模板：

```text
/goal 建立一本法語公版書的英語 EPUB，
從 {可靠公版來源 URL} 抓取原文，
按 template/epub_pipeline/common 與 template/epub_pipeline/fr-en 的總控提示詞串聯執行完整流程，
...
確認來源和版權風險後，選擇倉庫中匹配翻譯方向的模板；例如法語到英語用 template/epub_pipeline/fr-en，日語到西班牙語用 template/epub_pipeline/ja-es，英語到簡體中文用 template/epub_pipeline/en-zh-Hans。然後先複製 template/epub_pipeline/common，再覆蓋複製語言方向模板，建立 books/ 下的新書工程。
```
...
這是製作一本書時要複製使用的模板目錄。不要直接在這個目錄裡做某本書。`common/` 放通用 EPUB 流水線、版權核查、狀態機、構建腳本和製作規則；語言方向目錄放對應語言的提示詞、術語、文風和審校規則。普通參與者只需要知道“做新書時讓 AI 複製 common，再覆蓋對應語言方向模板，不要改模板原件”。

裡面包含：

- `template/epub_pipeline/README.md`：模板組織說明。
- `template/epub_pipeline/common/PIPELINE_SPEC.md`：流水線規範，說明每一步的輸入、輸出、狀態和目錄約定。
- `template/epub_pipeline/targets/zh-Hans/quality_framework/README.md`：簡體中文目標語言品質框架。
- `template/epub_pipeline/en-zh-Hans/README.md`：目前已有的英語到簡體中文模板說明。
...
這裡放每一本具體書的工程目錄。每本書都應該是從模板複製出來的獨立目錄，並按目標語言分組。

新目錄格式為：

`books/{目標語言}/{遞增數字}_{book_id_slug}/`

例如簡體中文譯本：

`books/zh-Hans/1_pg20923_a_negro_explorer_at_the_north_pole/`

樣例書基於 Project Gutenberg #20923，作者 Matthew A. Henson，中文題名暫作《黑人北極探險家》。它已經完成：
...
這裡放目標語言品質框架。例如 `template/epub_pipeline/targets/zh-Hans/quality_framework/` 保存簡體中文譯文品質標準、試譯規則、基準測試政策和章節門禁。
...
這裡放特定語言方向的調研產物和歷史探索文件。例如 `research/en-zh-Hans/book-discovery/` 保存英語到簡體中文方向的候選書審計表和檢索腳本。這個目錄裡的內容不是全域預設工具；其他語言方向應建立自己的調研目錄。
...
```text
請用 template/epub_pipeline/common 和匹配翻譯方向的語言模板建立一本新的書籍工程。
原文來源：https://www.gutenberg.org/ebooks/20923
翻譯方向：英語到簡體中文、法語到英語、日語到西班牙語等；請使用倉庫中實際存在的對應模板。
目標：生成可閱讀的目標語言 EPUB，並通過 EPUBCheck。
```

如果想參考已經跑通的樣例工程，可以補一句：

```text
請參考 books/zh-Hans/1_pg20923_a_negro_explorer_at_the_north_pole 的工程結構和質量控制方式。
```

AI 應該自動完成：

- 使用 `books/scripts/create_book_project.py` 複製模板。
- 建立 `books/{目標語言}/{遞增數字}_{book_id_or_author_title_slug}/`。
...
```text
books/{目標語言}/{遞增數字}_{book_id_or_author_title_slug}/
```
...
模板按語言方向組織。`common` 表示所有語言方向共享的 EPUB 製作流水線；`en-zh-Hans`、`fr-en`、`ja-es`、`zh-Hant-de` 這類目錄名表示具體翻譯方向。
...
- `prompts/` 中關於目標語言的表述。
- EPUB metadata 的語言欄位，例如 `es`、`ja`、`en-US`、`de`、`zh-Hant`。
...
語言模板目錄：template/epub_pipeline/{使用實際存在的語言方向模板，例如 en-zh-Hans；若要做法語到英語，可先新增 fr-en}
新書目錄：books/{請用書號或書名生成清晰目錄名}
原文來源：{填寫 Project Gutenberg / Wikisource / Standard Ebooks 等可靠來源 URL}
翻譯方向：{例如 英語到西班牙語 / 法語到日語 / 中文到英語 / 日語到德語}
...
- [LICENSE.zh-TW.md](../license/LICENSE.zh-TW.md)：公開授權和程式碼授權。
- [CONTRIBUTING.zh-TW.md](../license/CONTRIBUTING.zh-TW.md)：貢獻者授權與參與規則。
- [COMMERCIAL_LICENSE.zh-TW.md](../license/COMMERCIAL_LICENSE.zh-TW.md)：第三方商業使用授權說明。
