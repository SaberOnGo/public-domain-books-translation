...
每本书完成后，AI 必须复审所有阶段，提炼经验教训，并写入该书工程的复盘文件。若这些经验属于通用模板规则，必须给出模板版本递增建议；当用户明确要求更新模板时，通用规则写入 `template/epub_pipeline/common`，古希腊文到简体中文规则写入 `template/epub_pipeline/grc-zh-Hans`，特殊书型规则写入对应 `profiles/`，并递增版本号。
