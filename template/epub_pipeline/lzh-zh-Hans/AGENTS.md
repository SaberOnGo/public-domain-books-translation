# lzh-zh-Hans Agent Instructions / 文言文到现代简体中文 Agent 指令

This file is for AI agents using the Literary Chinese to Simplified Chinese EPUB template.

本文件供使用 `lzh-zh-Hans` 文言文到现代简体中文模板的 AI agent 读取。
...
- Create book projects through `books/scripts/create_book_project.py --source-target lzh-zh-Hans`; never write book-specific source text, translations, QA, metadata, or EPUB output into this template directory.
- 必须通过 `books/scripts/create_book_project.py --source-target lzh-zh-Hans` 创建书籍工程；不得把具体书籍的原文、译文、QA、metadata 或 EPUB 输出写入模板目录。
