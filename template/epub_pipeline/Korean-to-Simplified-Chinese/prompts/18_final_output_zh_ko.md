...
- 公版或授权项目：`output/release/book_vX.X.X.epub`、`output/release/release_notes.md`、`output/release/release_state.json`
...
6. 章节标题已按 `references/chapter_title_policy.md` 和 `references/korean_title_strategy.md` 检查：无半截标题、无韩文/朝鲜文原题/读音/罗马字/解释性括注污染，EPUB 目录使用短题名。
   - 若韩语/朝鲜语原章只有编号、无题分隔或简单题名，不得出现 AI 自拟的可见中文小标题；解释性概括只能放入 `title_note`、制作说明或 QA。
7. 文件体积合理，封面和字体未异常膨胀。
...
12. `release_notes.md` 或 `private_artifact_notes.md` 已记录发布/产物原因、问题点、修复方式、QA 证据、风险和下一轮迭代。
...
14. 如本书存在系统性精修问题，`goal/` 下已有本书目标或完成记录，且可复用经验已回填到 common、zh-Hans 或 Korean-to-Simplified-Chinese 模板。
15. 标题中的人名不计入“正文首次出现”：章节标题、副标题和目录题名只用中文译名或本书确定的中文呈现方式；韩文/朝鲜文原名、读音或罗马字只可放在正文第一次自然出现处、译注、术语表或书籍信息页。
...
- `output/book.epub`
- 公版或授权项目：`output/release/book_vX.X.X.epub`、`output/release/release_notes.md`
