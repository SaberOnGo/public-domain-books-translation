...
2. 判断可用的原文版本：优先韩国 Wikisource纯文本、可审计 HTML、国立国会图书馆扫描/文本或来源方明确的公版文本。
...
7. 生成 `metadata/rights_checklist.md`：
   ...
   - 美国/中国/常见地区公版风险初判
   - 是否允许进入翻译制作
8. 生成 `metadata/korean_source_profile.md`：
   - 来源版本、初出/出版年份和底本说明
   - 旧字体/新字体、旧拼写/现代拼写
   - 韩文/汉字混排、旁注、底本注、编者注、OCR 状态
   - 现代校订材料或参考材料的使用边界
9. 生成或初始化 `qa/textual/korean_textual_notes.md`：
   - 旧字/新字或旧拼写/现代拼写、韩文/汉字混排、异读、OCR 不确定和注记疑难
   - 如果没有发现疑难，写入明确的无疑难声明

## 硬规则 / Hard Rules

- 如果当前目录仍在 `template/epub_pipeline/common` 或 `template/epub_pipeline/ko-zh-Hans` 模板目录内，必须停止，回到 `00_orchestrator` 先复制模板到书籍工程。
- 如果版权状态不明，设置 `status=FAILED`，不得继续。
- 如果只找到商业电子书而无公版原文，不得继续。
- 不要把来源方自动生成的 EPUB 当唯一翻译底本；优先可审计的纯文本、HTML、扫描或可核查文本。
- 不要把现代校订说明、输入者注、韩国 Wikisource工作说明或版权说明混入作者正文。
- 如果 `metadata/korean_source_profile.md` 无法判定底本文字形态，设置 `status=FAILED`，不得继续。
