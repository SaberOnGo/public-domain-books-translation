# 韩语/朝鲜语到简体中文 EPUB 公版书翻译制作模板 / Korean to Simplified Chinese EPUB Translation Pipeline
...
4. 清洗、分章，并保留可追溯的原文证据。
5. 完成通用翻译研究、韩语/朝鲜语专项研究、本书专项研究、文体画像和预翻译试译。
...
目标不是“把韩文/朝鲜文换成中文”，而是产出来源清楚、文体判断明确、中文可读、有文学质感、EPUB 制作质量合格的简体中文正本书。

This template handles Korean source-language issues for Simplified Chinese EPUB production. It complements the target-language framework under `template/epub_pipeline/targets/zh-Hans/quality_framework/`.

## 唯一必须输入 / Required Inputs

- `TEMPLATE_ROOT`：语言方向模板目录，即 `template/epub_pipeline/ko-zh-Hans`。
...
- `SOURCE_URL`：韩语/朝鲜语公版或授权来源 URL，例如韩国 Wikisource、共享마당、国立中央图书馆、韩国近现代文学馆、Wikisource、Internet Archive 或其他可核查来源。
...
- `TEXT_FORM_POLICY`：用户对韩文/汉字混排、旧拼写、汉字词转写、专名译名或译注密度的明确要求。

## 模板保护 / Template Protection

严禁直接在模板原目录中制作具体书籍。执行任何书籍项目前，AI 必须先把 `template/epub_pipeline/common` 与 `template/epub_pipeline/ko-zh-Hans` 合并复制到独立书籍工程目录，例如：
...
## 韩语/朝鲜语专项规则 / Korean Rules

- 原文必须以韩语/朝鲜语公版底本为准；现代中文译本、出版社校注本、现代注释和现代译文不能作为翻译来源。
- 必须记录作者生卒年、初出/出版年份、来源站点版权口径，以及中国、韩国/朝鲜半岛、美国和常见 life+70 地区的初步版权风险。
- 必须说明所用文本是纯韩文、韩文/汉字混排、现代转写、旧拼写保留、扫描 OCR，还是人工校订文本。
- 旁注、编者注、底本注、输入者说明和校订者说明必须分清；不得把现代校订说明误当作者正文。
- 韩语/朝鲜语省主语、省宾语、敬语、称谓、视角转换、句末语气和暧昧心理描写，必须转成自然中文，同时保留叙述的不确定性。
- 官能、暴力、病态心理、权力关系或性别压迫题材不得被色情化、猎奇化或道德说教化；译文只呈现原作的叙述力度和文学判断。
- 人名、地名、作品名、时代词、衣物器物、学校/农村启蒙/殖民地制度词、佛教或基督教语境词必须建立统一译名和译注边界。
- 韩语/朝鲜语汉字词不能因为“看得懂”就照搬；必须判断现代中文语义是否漂移。
...
- `metadata/korean_source_profile.md`：底本文字形态、韩文/汉字混排、旧拼写、注记、OCR/校订状态。
- `metadata/book_specific_translation_research.md`：本书作者、时代、题材、文体、叙述策略和敏感题材边界。
- `metadata/style_profile.md`：中文文体画像。
- `glossary/terms.csv`：韩语/朝鲜语原词、汉字形/罗马字（必要时）、中文译名、说明、首次出现策略。
- `qa/textual/korean_textual_notes.md`：旧字、异体字、韩文/汉字混排、注记、OCR 或版本疑难。
...
1. `prompts/00_orchestrator_zh_ko.md`
2. `prompts/01_ingest_clean_zh_ko.md`
3. `prompts/02_split_zh_ko.md`
4. `prompts/03_global_translation_research_zh_ko.md`
5. `prompts/04_book_specific_research_zh_ko.md`
6. `prompts/05_pretranslation_trials_zh_ko.md`
7. `prompts/06_glossary_style_zh_ko.md`
8. `prompts/07_translate_chapters_zh_ko.md`
9. `prompts/08a_chapter_post_translation_control_zh_ko.md`
10. `prompts/08_review_fidelity_zh_ko.md`
11. `prompts/09_review_readability_imagery_zh_ko.md`
12. `prompts/10_review_terminology_zh_ko.md`
13. `prompts/11_chapter_quality_gate_zh_ko.md`
14. `prompts/13_preproduction_stage1_spec_zh_ko.md`
15. `prompts/14_preproduction_stage2_sample_zh_ko.md`
16. `prompts/15_full_book_production_zh_ko.md`
17. `prompts/16a_stratified_random_spotcheck.md`
18. `prompts/16_independent_review_agents_zh_ko.md`
19. `prompts/17_revision_routing_zh_ko.md`
20. `prompts/18a_release_versioning.md`
21. `prompts/18_final_output_zh_ko.md`
22. `prompts/19_retrospective_template_update_zh_ko.md`
...
- 未建立 `qa/textual/korean_textual_notes.md` 或明确说明无文本疑难，不得进入最终输出。
- 未完成韩语/朝鲜语源语言干扰研究、术语策略和称谓/敬语策略，不得批量翻译。
