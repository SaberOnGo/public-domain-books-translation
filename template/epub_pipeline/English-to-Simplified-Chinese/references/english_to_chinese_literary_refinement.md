...
- 人名、地名、船名、机构名必须统一。正文中的陌生人名或陌生地名不强制汉译，可以按 `glossary/proper_nouns.csv.display_policy` 保留英文原名，例如 `Professor Marvin`、`Grant Land`。标题、目录、页眉、图注索引等标题性位置不计入“正文首次出现”，统一使用锁定行的 `subsequent_rendering`：策略 `1`/`3`/`5` 通常用中文，策略 `2`/`4` 保留原名；不得在标题里追加正文首次出现括注。
