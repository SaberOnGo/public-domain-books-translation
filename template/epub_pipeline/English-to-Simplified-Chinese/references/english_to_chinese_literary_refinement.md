...
- 普通名词、器物名、衣物名、材料名和动作名应译成中文，不得写成 `source term（中文释义）`，也不得写成 `中文词（source term）`。正文首次出现保留英文原名的规则只适用于音译人名；普通名词能准确翻译时，不应附加原文词。
...
- 只有三类例外可以在正文使用短括注原词：一是不保留原词会造成明显误解；二是该原词本身是作者论证对象；三是学界译名分歧或本书术语体系需要读者当场知道原词。每个例外都必须在 `glossary/terms.csv`、本章译注或 `qa/chapter_controls/{chapter}.control.md` 中记录理由。不得为了“显得严谨”而让正文大面积夹杂英文、古英语、拉丁文或其他源语词。
