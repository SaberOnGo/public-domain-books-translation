...
2. 封面体积是否合理。
3. 公版或授权项目的版本说明是否包含 `LifeBook 书坊 + 个人名`、译制时间、公版来源 URL、公版说明；`private_use` 项目是否改用 `参考public-domain-books-translation 开源项目 个人自制`，并去掉公版说明。
4. 是否仍有旧品牌名如 `LifeBook 翻译组`。
5. 是否锁死难看字体或嵌入完整超大中文字体。
6. 章节标题在手机窄屏下是否过大。
7. `第X章` 与章节说明字号是否协调。
8. EPUB `nav.xhtml` 是否使用短目录题名，而不是纸书目录式长标题链。
9. 页面主标题和副标题是否按 `production_spec.md` 呈现，是否存在半截标题、日文原题/读音/括注污染目录，或解释性信息挤入可见标题。
