# ja-zh-Hans Agent Instructions / 日语到简体中文模板 Agent 指令

This file is for AI agents using the `ja-zh-Hans` template.

本文件供使用 `ja-zh-Hans` 模板的 AI agent 读取。
...
- Create each new book project with `books/scripts/create_book_project.py`; it copies `template/epub_pipeline/common` first, then overlays `template/epub_pipeline/ja-zh-Hans` into `books/zh-Hans/{number}_{目标语言书名}_{目标语言作者名}/`.
- 必须用 `books/scripts/create_book_project.py` 创建每本新书；脚本会先复制 `template/epub_pipeline/common`，再覆盖复制 `template/epub_pipeline/ja-zh-Hans` 到 `books/zh-Hans/{number}_{目标语言书名}_{目标语言作者名}/` 书籍工程。
...
- Preserve Japanese source evidence, source edition information, text-form information, and rights checks before translation. Public projects require public-domain or licensed source evidence; private-use projects require a user-provided local source file and `metadata/private_use_declaration.md`.
- 翻译前必须保留日语来源证据、来源版本信息、底本文字形态和版权核查。公开项目必须有公版或授权来源证据；私人自用项目必须有用户提供的本地书源文件和 `metadata/private_use_declaration.md`。
...
- A modern Japanese scholarly edition, modern translation, Chinese translation, English translation, or commentary may be used only as a reference when its copyright status and use boundary are recorded; it must not be copied into the translation.
- 现代日文校注本、现代译本、中文译本、英译本或研究注释，只能在版权状态和使用边界记录清楚后作为参考；不得复制进译文。

- Translation must be based on the Japanese source, not on a Chinese or English pivot translation.
- 翻译必须从日语底本出发，不得从中文或英文转译。

- Record and preserve old/new character forms, historical kana, furigana, source notes, editorial notes, OCR uncertainty, and ambiguous readings.
- 必须记录并保留旧字体/新字体、历史假名遣、振假名、底本注、编者注、OCR 不确定和歧义读法。

- Before batch translation, create `metadata/japanese_source_profile.md` and `qa/textual/japanese_textual_notes.md`.
- 批量翻译前必须创建 `metadata/japanese_source_profile.md` 和 `qa/textual/japanese_textual_notes.md`。

- Do not silently normalize Japanese names, place names, era terms, art terms, Buddhist terms, clothing/object terms, or variant readings. Record the Chinese rendering policy.
- 不得静默统一日语人名、地名、时代词、艺道词、佛教词、衣物器物词或异体读法；必须记录中文译名策略。
