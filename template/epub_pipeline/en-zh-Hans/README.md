...
12. 每轮精校后生成随机段落抽检样本，由 2 个独立 Agent 各抽检不少于 10 个中文正文段落。
13. 根据评审和随机抽检结果回退到任意前置阶段返工。
14. 最终输出 EPUB。
15. 全阶段复审，总结经验教训，必要时递增模板版本。
...
- 每轮精校后未完成双 Agent 随机段落抽检，不得宣布精校完成。
- 随机抽检中任一 Agent 平均分 < 75、任一单段 < 70，或指出读不懂、事实误解、英文句法硬搬、无依据润饰、术语/专名/译注错误，必须回退精校或更早阶段。
- 评审未通过，必须回退返工。
