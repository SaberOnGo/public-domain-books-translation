...
5. 术语与专名：全章一致；历史制度、身份称谓、组织名、变量名等以中文为主；必要原词优先放本章译注、章末注或术语表。正文注号如 `[1]` / `（1）` 必须与注释对应，不能让正文被英文或源语括注打断。必须按 `glossary/terms.csv.forbidden_body_renderings` 扫描正文并记录结果。
...
7. 注释：解释足够、不过密、不打断阅读；脚注标记不与正文黏连；读者不查外部资料也能理解关键处。
