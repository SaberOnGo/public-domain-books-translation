...
每次只翻译当前章节中分配的 canonical short-paragraph units。翻译 prompt 保持瘦身：当前原文、必要上下文、文体画像 5-8 条、当前术语。模型只生成 unit 的 `target_template` 更新并写入 worker-owned chapter patch，不混入 QA、prompt、制作说明或分析；不得直接写入任一投影目录。
