...
- 保持标题和段落结构。
- 章节标题必须按 `references/sanskrit_title_strategy.md` 处理；不得把现代编辑者目录或参考译本目录机械翻成中文破折号长链。
- 强制规则：章节标题、副标题和 EPUB 目录题名里的人名不算“正文首次出现”。标题只使用中文译名，不得追加梵语原名、拉丁化转写或外文括注；原名和转写必须放到正文第一次自然出现该人名的位置、译注或术语表。
...
- 忠实事实和语气。
- 必须从梵语底本翻译；第二语言译本只能用于疑难校读，不得直接转译。
...
- `prompts/08a_chapter_post_translation_control_zh_sa.md`
