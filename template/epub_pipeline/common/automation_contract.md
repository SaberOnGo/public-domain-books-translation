...
- 若发现 P0/P1/P2、读者不可理解、事实/术语/文字接口错误、模板硬门禁失败、总评分低于项目阈值，或总评分低于 75 分，必须修复并追加一次同节点复查。
- 最近一次同节点复查无未关闭阻塞问题，并且“无问题或总评分不小于 75 分”；若 profile/项目规则更严格，按更严格规则。分数不能抵消 P0/P1/P2、读者难以理解、事实/术语/文字接口错误或模板硬门禁失败。
...
- `preflight:template` 必须在发现 `chapters/final/*.md` 时检查对应章节 control 与 gate 是否存在且 PASS。失败时不得构建 EPUB、创建 release 或 private artifact。
