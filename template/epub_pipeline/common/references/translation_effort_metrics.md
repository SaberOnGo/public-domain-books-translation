...
- `template/epub_pipeline/common/scripts/plan_parallel_translation.py`: provider-neutral weighted-workload and capability-cap planner.
...
npm run translation:orchestration:plan
```
...
The assessment also records `parallelization_profile`: a weighted source-unit estimate, chapter count, structural risk aggregates, and the next planning command. Page count is not the sole planning basis. After canonical units and all global translation resources are locked, `translation:orchestration:plan` prefers actual chapter-level canonical weights and combines them with live client, user, rate, budget, and quality caps.

评估还会记录 `parallelization_profile`：加权原文单位估计、章节数、结构风险聚合和下一条规划命令。页数不能作为唯一规划依据。canonical units 与所有全局翻译资源锁定后，`translation:orchestration:plan` 优先使用真实逐章 canonical 权重，再与客户端、用户、速率、预算和质量上限合并。
...
- `chapter_count`.
- `parallelization_profile.weighted_source_unit_estimate` and `planning_basis`.
...
并行规划必须遵守 `adaptive_parallel_orchestration.md`：能力未知时派生 worker 为 0；GPT 家族硬上限 4，非 GPT 硬上限 8；小书不并行；大书先用最多 2 个 worker 做 pilot，质量达标后再扩容。planner 只生成角色和章节工作计划，不替代活动客户端的真实 worker 创建，也不把厂商具体模型写死进通用代码。
