...
原言語が英語、対象言語が簡体字中国語の場合は、既定で `edition_type: bilingual_parallel` とし、対象言語のみの簡体字中国語 EPUB と英中対照 EPUB の両方を生成してください。他の言語ペアでは、私が `请输出 edition_type: bilingual_parallel，同时生成目标语言版 EPUB 和源语言-目标语言双语对照版 EPUB。` と明示した場合だけ対照版を出力してください。
...
原言語が英語、対象言語が簡体字中国語の場合は、既定で `edition_type: bilingual_parallel` とし、対象言語のみの簡体字中国語 EPUB と英中対照 EPUB の両方を private artifact として生成してください。他の言語ペアでは、私が `请输出 edition_type: bilingual_parallel，同时生成目标语言版 EPUB 和源语言-目标语言双语对照版 EPUB。` と明示した場合だけ対照版を出力してください。
