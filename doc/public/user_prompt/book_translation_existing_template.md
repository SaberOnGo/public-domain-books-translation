...
除此之外，源语言、可靠公版来源 URL、source-target 语言方向、目标语言标签、项目 slug、是否需要 profile、建书目录编号，都必须由你自动判断、自动查找、自动记录。不要要求用户补充这些技术字段，除非版权状态或来源权利无法确认。

任务目标：
严格依据当前仓库规则，创建并完成这本书的公版 EPUB 翻译项目。最终必须生成 `books/{target}/{number}_{slug}/output/release/` 下 latest_status=PASS 的可发布 EPUB。`output/book.epub` 不能单独作为完成依据。
...
5. 若 SOURCE_URL 未由用户提供，必须自动查找可靠公版来源，例如 Project Gutenberg、青空文库、Wikisource、Internet Archive、Gallica、国家图书馆/大学馆藏等。
6. 若用户给的是本地文件，必须核查本地文件来源、版权状态和提交权利；本地文件存在不等于可发布。版权不清楚时停止。
...
9. 必须使用 `books/scripts/create_book_project.py` 创建 `books/{target}/{next_number}_{slug}/`。slug 由你根据书名和作者自动生成。
