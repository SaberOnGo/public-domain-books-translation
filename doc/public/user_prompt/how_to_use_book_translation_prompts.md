...
这套 prompt 的目标是：用户不需要懂 EPUB、source-target、slug、模板、抽检、release。用户只要告诉 AI 两件事：
...
请执行 doc/public/user_prompt/book_translation_existing_template.md
```
...
请执行 doc/public/user_prompt/book_translation_new_template.md
```
...
请先检查仓库是否已有对应源语言到目标语言模板。
如果已有，执行 doc/public/user_prompt/book_translation_existing_template.md。
如果没有，执行 doc/public/user_prompt/book_translation_new_template.md。
```
