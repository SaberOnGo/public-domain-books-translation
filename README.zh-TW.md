...
按 epub_pipeline_template_zh_en 的 00_orchestrator_zh_en.md 串聯執行當前完整流程，
...
確認來源和版權風險後，再用 epub_pipeline_template_zh_en 建立 books/ 下的新書工程。
```
...
這是製作一本書時要複製使用的模板目錄。不要直接在這個目錄裡做某本書。這個目錄主要是給 AI 讀取和執行的，人類一般不需要逐個檔案去看；普通參與者只需要知道“做新書時讓 AI 複製它，不要改模板原件”。

裡面包含：

- `README_ZH_EN.md`：模板說明。
- `MASTER_PROMPT_ZH_EN.md`：啟動 AI 製作一本書時可以使用的主控提示詞。
- `PIPELINE_SPEC_ZH_EN.md`：流水線規範，說明每一步的輸入、輸出、狀態和目錄約定。
...
```text
請用 epub_pipeline_template_zh_en 建立一本新的書籍工程。
...
當前模板名裡有 `zh_en`，主要表示模板文件是中英雙語，並且第一本樣例是英文公版書譯成中文。
...
`epub_pipeline_template_zh_en/` 是模板。做新書時讓 AI 複製它，不要直接在裡面寫某本書的資料。普通參與者不需要閱讀模板裡的每個提示詞檔案。
...
模板目錄：epub_pipeline_template_zh_en
...
本專案中產生的譯文、註釋、封面、排版和 EPUB 打包檔案可能有各自的權利狀態。正式公開發布前，專案維護者應明確具體授權方式。
