...
确认来源和版权风险后，选择仓库中匹配翻译方向的模板；例如法语到英语用 template/epub_pipeline/fr-en，日语到西班牙语用 template/epub_pipeline/ja-es，英语到简体中文用 template/epub_pipeline/en-zh-Hans。然后先复制 template/epub_pipeline/common，再覆盖复制语言方向模板，创建 books/ 下的新书工程。
```
...
这是制作一本书时要复制使用的模板目录。不要直接在这个目录里做某本书。`common/` 放通用 EPUB 流水线、版权核查、状态机、构建脚本和制作规则；语言方向目录放对应语言的提示词、术语、文风和审校规则。普通参与者只需要知道“做新书时让 AI 复制 common，再覆盖对应语言方向模板，不要改模板原件”。
...
Node.js 工具依赖统一放在 `books/`：先在 `books/` 目录运行一次 `npm install`，再进入具体书籍目录运行 `npm run ...`。不要在每本书目录下重复创建 `node_modules/`；除非某本书确实需要私有工具链，并在该书 QA 或复盘中说明原因。
